defmodule TesterWeb.Scan4transportLiveTest do
  ...
  describe "Index" do
    ...
    test "updates scan4transport in listing", %{conn: conn, scan4transport: scan4transport} do
      ...
      assert index_live |> element("#scan4transport-#{scan4transport.id} a", "Edit") |> render_click() =~
               "Edit Scan4transport"
    ...
    test "deletes scan4transport in listing", %{conn: conn, scan4transport: scan4transport} do
      ...
      assert index_live |> element("#scan4transport-#{scan4transport.id} a", "Delete") |> render_click()
  ...
  describe "Show" do
    ...
    test "displays scan4transport", %{conn: conn, scan4transport: scan4transport} do
      {:ok, _show_live, html} = live(conn, Routes.scan4transport_show_path(conn, :show, scan4transport))
    ...
    test "updates scan4transport within modal", %{conn: conn, scan4transport: scan4transport} do
      {:ok, show_live, _html} = live(conn, Routes.scan4transport_show_path(conn, :show, scan4transport))
